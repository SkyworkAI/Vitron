...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, regions=None):
        ...
        if regions is not None and region_features is not None:
            ...
            dummy_region_features = torch.randn(1).to(self.device)
            return image_features.to(self.device), dummy_region_features

    def encode_videos(self, videos, regions=None):  # [mini_b, c, t, h, w]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if regions is not None:
            ...
            regions_imgages_minibatch = [regions[idx] for idx in image_idx] if len(image_idx) > 0 else []
            regions_videos_minibatch = [regions[idx] for idx in video_idx] if len(video_idx) > 0 else []
            tmp_image_features = [None] * (len(image_idx) + len(video_idx))
            temp_region_features = [None] * (len(image_idx) + len(video_idx))
            # print('images_minibatch shape: ', images_minibatch.shape)  # [1, 3, 224, 224]
            if getattr(images_minibatch, 'ndim', 0) == 4:  # batch consists of images, [mini_b, c, h, w]
                if image_tower is not None:
                    image_features_minibatch, region_features_minibatch = self.encode_images(images_minibatch, regions_imgages_minibatch)  # [mini_b, l, c]
            ...
            if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b, c, t, h, w]
                video_features_minibatch, region_features_minibatch = self.encode_videos(videos_minibatch)  # fake list [mini_b, t, l, c]
                for i, pos in enumerate(video_idx):
                    ...
                    temp_region_features[pos] = [region_features_minibatch[i][j] for j in range(t)]
                    # temp_region_features[pos] = region_features_minibatch[i]
            ...
            # print('input_ids 22: ', input_ids)
            # print('labels 22: ', labels)
            ...
            for batch_idx, cur_input_ids in enumerate(input_ids):
                ...
                for i, _token_indices in zip(range(num_images + num_objs + 1), _specfical_token_indices[1:]):
                    ...
                    if _token_indices in image_token_indices:
                        ...
                        cur_region_idx = region_token_indices.index(_token_indices)
                        cur_mm_features = region_features[cur_region_idx]
                        cur_new_input_embeds.append(cur_mm_features)
                        cur_new_labels.append(torch.full((cur_mm_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                        # print("cur_region_features: ", cur_mm_features)
