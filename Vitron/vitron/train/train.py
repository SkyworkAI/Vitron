...
from videollava.mm_utils import tokenizer_image_token, tokenizer_image_region_token
...
def expand2squareWregion(pil_img, background_color, region):
    ...
    if width == height:
        ...
        result = Image.new(pil_img.mode, (width, width), background_color)
        result.paste(pil_img, (0, (width - height) // 2))
        res_region = [region[0], region[1]+((width - height) // 2), region[2], region[3]]
        return result, res_region
    else:
        result = Image.new(pil_img.mode, (height, height), background_color)
        result.paste(pil_img, ((height - width) // 2, 0))
        res_region = [region[0]+((width - height) // 2), region[1], region[2], region[3]]
        return result, res_region

def preprocess_region(region, image_size, target_size):
    x, y, w, h = region
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
                ...
                if self.data_args.image_aspect_ratio == 'pad':
                    if 'bbox' in sources[0]:
                        ...
                        image, region = zip(*[expand2squareWregion(i, tuple(int(x * 255) for x in image_processor.image_mean), region_ann) for i in image])
