...
from modules.GLIGEN.demo.gligen.ldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like
...
class PLMSSampler(object):
    ...
    def plms_sampling(self, shape, input, uc=None, guidance_scale=1, mask=None, x0=None):
        ...
        if img is None:
            if mask is None:
                ...
                img_shape = (mask_shape[0], shape[1], mask_shape[-2], mask_shape[-1])
        ...
        for i, step in enumerate(time_range):
            ...
            if mask is not None:
                ...
                if img.size(2) != mask.size(2):
                    mask = F.interpolate(mask, size=(img.size(2), mask.size(3)), mode='bilinear', align_corners=False)
                if img.size(2) != img_orig.size(2):
                    img_orig = F.interpolate(img_orig, size=(img.size(2), img_orig.size(3)), mode='bilinear',
                                             align_corners=False)
    ...
    def p_sample_plms(self, input, t, index, guidance_scale=1., uc=None, old_eps=None, t_next=None):
        ...
        def get_x_prev_and_pred_x0(e_t, index):
            ...
            if x.size(2) != e_t.size(2):
                x = F.interpolate(x, size=(e_t.size(2), x.size(3)), mode='bilinear', align_corners=False)
