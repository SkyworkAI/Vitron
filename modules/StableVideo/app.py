...
from modules.StableVideo.cldm.model import create_model, load_state_dict
from modules.StableVideo.ldm.models.diffusion.ddim import DDIMSampler
...
class StableVideo:
    ...
    def load_video(self, video_path, video_name):
        self.data = AtlasData(video_path, video_name)
    ...
    @torch.no_grad()
    def render_without_mask(self, f_atlas, b_atlas):
        # foreground
        if f_atlas == None:
            f_atlas = transforms.ToTensor()(self.f_atlas_origin).unsqueeze(0).cuda()
        else:
        ...
        f_atlas = torch.nn.functional.pad(
            ...
            pad=(
                self.data.foreground_atlas_bbox[1],
                self.data.foreground_grid_atlas.shape[-1] - (
                            self.data.foreground_atlas_bbox[1] + self.data.foreground_atlas_bbox[3]),
                self.data.foreground_atlas_bbox[0],
                self.data.foreground_grid_atlas.shape[-2] - (
                            self.data.foreground_atlas_bbox[0] + self.data.foreground_atlas_bbox[2]),
            ),
            mode="replicate",
        )
        foreground_edit = F.grid_sample(
        ...
        foreground_edit = foreground_edit.squeeze().t()  # shape (batch, 3)
        foreground_edit = (
            foreground_edit.reshape(self.data.config["maximum_number_of_frames"], self.data.config["resy"],
                                    self.data.config["resx"], 3)
            .permute(0, 3, 1, 2)
            .clamp(min=0.0, max=1.0)
        )
        # background
        ...
        background_edit = background_edit.squeeze().t()  # shape (batch, 3)
        background_edit = (
            background_edit.reshape(self.data.config["maximum_number_of_frames"], self.data.config["resy"],
                                    self.data.config["resx"], 3)
            .permute(0, 3, 1, 2)
            .clamp(min=0.0, max=1.0)
        )
        ...
        if not os.path.exists('temp'):
            os.mkdir('temp')
        os.mkdir(f"log/{id}")
        save_name = os.path.join('temp', next(tempfile._get_candidate_names()) + '.mp4')
        imageio.mimwrite(save_name, (255 * output_video.detach().cpu()).to(torch.uint8).permute(0, 2, 3, 1))
...
if __name__ == '__main__':
    with torch.cuda.amp.autocast():
        ...
        block.launch(server_port="80",server_name="0.0.0.0",share=True)
