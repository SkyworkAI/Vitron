...
from vitron.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, MAX_IMAGE_LENGTH, \
    MAX_VIDEO_LENGTH
from vitron.constants import OBJS_TOKEN_INDEX, DEFAULT_OBJS_TOKEN, DEFAULT_OBJS_PATCH_TOKEN,DEFAULT_OBJS_START_TOKEN, \
    DEFAULT_OBJS_END_TOKEN, OBJS_PLACEHOLDER 
from torch.utils.data import Dataset
from vitron.train.llava_trainer import LLaVATrainer

from vitron import conversation as conversation_lib
from vitron.model import *
from vitron.mm_utils import tokenizer_image_token, tokenizer_image_region_token, show_image_with_bboxes, preprocess_region

from PIL import Image
from vitron.utils import order_pick_k
...
class ModelArguments:
    ...
    tune_region_mlp_adapter: bool = field(default=False)
    pretrain_region_mlp_adapter: Optional[str] = field(default=None)
    # ===================================================================
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    freeze_region_mlp_adapter: bool = field(default=False)
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler', 'region_extractor']
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        return
    # if getattr(trainer.args, "tune_region_mlp_adapter", False):
    #     # Only save Adapter
    #     keys_to_match = ['mm_projector']
    #     if getattr(trainer.args, "use_im_start_end", False):
    #         keys_to_match.extend(['embed_tokens', 'embed_in'])
    ...
    if trainer.deepspeed:
        print('trainer.deepspeed is true')
...
def smart_tokenizer_and_add_tokens(
    new_tokens: Union[str, List[Union[str]]],
    tokenizer: transformers.PreTrainedTokenizer,
    model: transformers.PreTrainedModel,
):
    """
    Add tokens into tokenizer, not specfical tokens
    """
    rank0_print(f'Add new tokens {new_tokens}')
    rank0_print(f'before adding {len(tokenizer)}')
    num_new_tokens = tokenizer.add_tokens(new_tokens)
    rank0_print(f'add {num_new_tokens} new tokens')
    rank0_print(f'after adding {len(tokenizer)}')
    model.resize_token_embeddings(len(tokenizer))

    if num_new_tokens > 0:
        input_embeddings = model.get_input_embeddings().weight.data
        rank0_print(input_embeddings.requires_grad)
        output_embeddings = model.get_output_embeddings().weight.data
        rank0_print(output_embeddings.requires_grad)
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            region_replace_token = DEFAULT_OBJS_TOKEN 
            ...
            sentence['value'] = sentence['value'].replace(DEFAULT_OBJS_TOKEN, region_replace_token)
...
def preprocess_v1(
    ...
    if has_image:
        if has_region:
            input_ids = torch.stack([tokenizer_image_region_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
        else:
            input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    else:
        input_ids = tokenizer(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                if has_region:
                    round_len = len(tokenizer_image_region_token(rou, tokenizer))
                    instruction_len = len(tokenizer_image_region_token(parts[0], tokenizer)) - 2
                else:
                    round_len = len(tokenizer_image_token(rou, tokenizer))
                    instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) - 2
            else:
...
def preprocess(
    ...
    """
    # print(conversation_lib.default_conversation.sep_style)  # conv_vicuna_v1
    # print(conversation_lib.default_conversation.version)  # v1
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.PLAIN:
        return preprocess_plain(sources, tokenizer)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, has_image=has_image, has_region=has_region)
    ...
    def get_tokenize_image_region_len(prompts):
        return [len(tokenizer_image_region_token(prompt, tokenizer)) for prompt in prompts]

    if has_image:
        if has_region:
            input_ids = [tokenizer_image_region_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations]
        else:
            input_ids = [tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations]
    else:
    ...
    for target, source in zip(targets, sources):
        if has_image:
            if has_region:
                tokenized_lens = get_tokenize_image_region_len([header] + [s["value"] for s in source])
            else:
...
def expand2squareWregion(pil_img, background_color, region):
    width, height = pil_img.size
    if width == height:
        return pil_img, region
    elif width > height:
        result = Image.new(pil_img.mode, (width, width), background_color)
        result.paste(pil_img, (0, (width - height) // 2))
        res_region = [region[0], region[1]+((width - height) // 2), region[2], region[3]+((width - height) // 2)]
        return result, res_region
    else:
        result = Image.new(pil_img.mode, (height, height), background_color)
        result.paste(pil_img, ((height - width) // 2, 0))
        res_region = [region[0]+((height - width) // 2), region[1], region[2]+((height - width) // 2), region[3]]
        return result, res_region
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        # self.list_data_dict = random.sample(list_data_dict, 42880)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
                ...
                data_dict = preprocess(sources, self.tokenizer, has_image=True, has_region=True)
                ...
                if 'bbox' in sources[0]:
                    ori_image_size = [[600, 480] for vid in video_file]
                    region_ann = [self.list_data_dict[i]['bbox']]
                    region = [preprocess_region(bbox, ori_im_size, [224, 224]) for bbox, ori_im_size in zip(region_ann, ori_image_size)]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            ...
            if 'image' in self.list_data_dict[i] or 'video' in self.list_data_dict[i]:
                ...
                if 'bbox' in self.list_data_dict[i] and region is not None:
                    data_dict['region'] = region
                else: 
                    print(self.list_data_dict[i])
                    print(region)
                    print('region is nonnnnnnnnnnnnnn-------')
                    exit(0)
                ...
                data_dict['region'] = [[0,0, 224,224]]  # dummy region
...
def compute_metrics(pred):
    ...
    chosen_tokens = torch.max(pred_ids, dim=-1)[1][:, 1:-1]  # [B, S-1]
    labels = labels_ids[:, 2:]
...
def train():
    ...
    rank0_print("locak ran: ", local_rank)
    ...
    # model.enable_input_rxequire_grads = True
    if training_args.gradient_checkpointing:
        if hasattr(model, "enable_input_require_grads"):
            rank0_print("enable input require grads ...")
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model),  # find_all_linear_names(model), ['q_proj', 'k_proj', 'v_proj', 'o_proj']
    ...
    if model_args.version == "v0":
        ...
        rank0_print(tokenizer.pad_token)
        tokenizer.pad_token = tokenizer.unk_token
    ...
    # =============================================================================================================
        ...
        model.config.freeze_region_mlp_adapter = training_args.freeze_region_mlp_adapter
        if training_args.freeze_region_mlp_adapter:
            for p in model.get_model().region_extractor.parameters():
                p.requires_grad = False

        if training_args.bits in [4, 8]:
            model.get_model().region_extractor.to(dtype=compute_dtype, device=training_args.device)
